void HTTP_init(void) {
  ...
  HTTP.on("/color", handle_color);
  HTTP.on("/bright", handle_bright);
  HTTP.on("/led", handle_led);
  HTTP.begin();
...
void handle_color() {
  color = map(HTTP.arg("color").toInt(),0,360,0,255);
  colorLED = HTTP.arg("color").toInt();
  red = color;
  green = 255;
  blue = max_bright;
...
void handle_bright() {
  max_bright = HTTP.arg("bright").toInt();
  LEDS.setBrightness(max_bright);
  HTTP.send(200, "text/plain", "OK");
  }

void handle_led() {
  ledMode = HTTP.arg("led").toInt();
  HTTP.send(200, "text/plain", "OK");
  }
