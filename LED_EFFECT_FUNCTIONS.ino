...
void rainbow_fade() {                         //-m2-FADE ALL LEDS THROUGH HSV RAINBOW
  ...
  for (int idex = 0 ; idex < LED_COUNT; idex++ ) {
    leds[idex] = CHSV(ihue, thissat, max_bright);
  }
  LEDS.show();
  delay(thisdelay);
}
...
void color_bounce() {                        //-m5-BOUNCE COLOR (SINGLE LED)
  ...
  for (int i = 0; i < LED_COUNT; i++ ) {
    if (i == idex) {
      leds[i] = CHSV(thishue, thissat, max_bright);
    }
    else {
      leds[i] = CHSV(0, 0, 0);
    }
...
void ems_lightsONE() {                    //-m7-EMERGENCY LIGHTS (TWO COLOR SINGLE LED)
  ...
  for (int i = 0; i < LED_COUNT; i++ ) {
    if (i == idexR) {
      ...
      leds[i] = CHSV(thathue, thissat, max_bright);
    }
    else {
      leds[i] = CHSV(0, 0, 0);
    }
...
void ems_lightsALL() {                  //-m8-EMERGENCY LIGHTS (TWO COLOR SOLID)
  ...
  int idexB = antipodal_index(idexR);
  int thathue = (thishue + 160) % 255;
  leds[idexR] = CHSV(thishue, thissat, max_bright);
  leds[idexB] = CHSV(thathue, thissat, max_bright);
  LEDS.show();
  delay(thisdelay);
}

void flicker() {                          //-m9-FLICKER EFFECT
  int random_bright = random(0, max_bright);
...
void pulse_one_color_all() {              //-m10-PULSE BRIGHTNESS ON ALL LEDS TO ONE COLOR
  ...
  one_color_all_HSV(thishue, thissat, ibright);
  LEDS.show();
  delay(thisdelay);
}
...
void pulse_one_color_all_rev() {           //-m11-PULSE SATURATION ON ALL LEDS TO ONE COLOR
  ...
  for (int idex = 0 ; idex < LED_COUNT; idex++ ) {
    leds[idex] = CHSV(thishue, isat, max_bright);
  }
  LEDS.show();
  delay(thisdelay);
}
...
void random_incolor() {                       //QUICK 'N DIRTY RANDOMIZE TO GET CELL AUTOMATA STARTED
  int temprand;
  for (int i = 0; i < LED_COUNT; i++ ) {
    temprand = random(0, 100);
    if (temprand > 50) {
      leds[i] = CHSV(thishue, 255, max_bright);
    }
    if (temprand <= 50) {
...
void rule30() {                          //-m13-1D CELLULAR AUTOMATA - RULE 30 (RED FOR NOW)
  ...
  for (int i = 0; i < LED_COUNT; i++ ) {
    ...
    if (ledsX[iCCW][0] > y && ledsX[i][0] > y && ledsX[iCW][0] > y) {
      leds[i] = CHSV(thishue, thissat, 0);
    }
    if (ledsX[iCCW][0] > y && ledsX[i][0] > y && ledsX[iCW][0] <= y) {
      leds[i] = CHSV(thishue, thissat, max_bright/2);
    }
    if (ledsX[iCCW][0] > y && ledsX[i][0] <= y && ledsX[iCW][0] > y) {
      leds[i] = CHSV(thishue, thissat, max_bright/2);
    }
...
void physcoStrobe() {
int thathue = (thishue + 160) % 255;
    one_color_all_HSV(thishue, 255, 255);
    LEDS.show(); delay(20);
    one_color_all_HSV(thishue, 255, 0);
    LEDS.show(); delay(80);
    one_color_all_HSV(thathue, 255, 255);
    LEDS.show(); delay(20);
    one_color_all_HSV(thathue, 255, 0);
    LEDS.show(); delay(80);
}
...
void HSV_to_RGB(long int *hue, long int *saturation, long int *value)
{
float f,p,q,t;
  int i;
  float h,s,v;
  ...
  s = map(*saturation,0,255,0,100);
  v = map(*value,0,255,0,100);
  ...
  switch(i%6) {
    case 0:
      (*hue) = round(255*v);
      (*saturation) = round(255*t);
      (*value) = round(255*p);
      break;
    case 1:
      (*hue) = round(255*q);
      (*saturation) = round(255*v);
      (*value) = round(255*p);
      break;
    case 2:
      (*hue)= round(255*p);
    ...
    case 3:
      (*hue) = round(255*p);
      (*saturation) = round(255*q);
      (*value) = round(255*v);
      break;
    case 4:
      (*hue) = round(255*t);
      (*saturation) = round(255*p);
      (*value) = round(255*v);
      break;
    default: // case 5:
      (*hue) = round(255*v);
